...
const string2Object = str => {
    ...
    // css 转换报错时，使用原来的值
    try {
        const cssObject = taro_css_to_react_native_1.transformCSS(entries);
        return cssObject;
    }
    catch (_a) {
        return str;
...
const object2Expression = (template, cssObject) => {
    ...
    return (_a = ast.declarations[0]) === null || _a === void 0 ? void 0 : _a.init;
};
...
function default_1(babel) {
    ...
    function getMatchRule(enableMultipleClassName) {
...
function importDeclaration(astPath, state, t) {
    ...
    const ext = path_1.default.extname(sourceValue);
    const styleSheetIdentifiers = file.get('styleSheetIdentifiers') || [];
    const cssModuleStylesheets = file.get('cssModuleStylesheets') || [];
    ...
    if (isStyle(sourceValue)) {
        ...
        if (isModuleSource(sourceValue) && enableCSSModule) {
            ...
            const cssFileName = path_1.default.basename(sourceValue);
            const cssFileBaseName = path_1.default.basename(cssFileName, ext);
            // 引入样式对应的变量名
            ...
            if (styleSheetName) {
                ...
                styleSheetName = camelize_1.default(`${cssFileBaseName}${ext}_${NAME_SUFFIX}`);
